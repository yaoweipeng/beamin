...
MODULE shared_data
  ...
  REAL(num) :: dt, t_end, time, dt_multiplier, dt_laser, dt_beam, dt_plasma_frequency
  ...
  !----------------------------------------------------------------------------
  TYPE beam_block
    ! Boundary to which beam is attached
    INTEGER :: boundary
    ! A unique id number for the beam (not used directly by EPOCH)
    ! Only used if hard coding time profiles
    INTEGER :: id
    REAL(num), DIMENSION(:), POINTER :: profile
    REAL(num), DIMENSION(:), POINTER :: phase
    ...
    TYPE(beam_block), POINTER :: next
  END TYPE beam_block

  TYPE(beam_block), POINTER :: beam_x_min, beam_x_max
  TYPE(beam_block), POINTER :: beam_y_min, beam_y_max
  INTEGER :: n_beam_x_min, n_beam_x_max
  INTEGER :: n_beam_y_min, n_beam_y_max
  LOGICAL, DIMENSION(2*c_ndims) :: add_beam = .FALSE.
  !-----------------------------------------------------------------------------
