...
CONTAINS
  ...
  ! yaowp @ 2017.3.13
  !----------------------------------------------------------------------
  SUBROUTINE init_beam(boundary, beam)

    INTEGER, INTENT(IN) :: boundary
    ! except for "laser_block", "deck_laser_block" is also needed.
    ! todo "deck_laser_block"
    TYPE(beam_block), INTENT(INOUT) :: beam  ! in "shared_data.f90", under "TYPE laser_block" 

    beam%boundary = boundary
    beam%id = -1
    beam%use_time_function = .FALSE.
    beam%use_phase_function = .FALSE.
    beam%use_profile_function = .FALSE.
    beam%amp = -1.0_num
    beam%omega = -1.0_num
    beam%pol_angle = 0.0_num
    beam%t_start = 0.0_num
    beam%t_end = t_end
    NULLIFY(beam%profile)
    NULLIFY(beam%phase)
    NULLIFY(beam%next)

    CALL allocate_with_boundary(beam%profile, boundary)
    CALL allocate_with_boundary(beam%phase, boundary)
    beam%profile = 1.0_num
    beam%phase = 0.0_num
  ...
  SUBROUTINE deallocate_laser(laser)
    ...
    IF (ASSOCIATED(laser%phase)) DEALLOCATE(laser%phase)  ! twice???
  ...
  !------------------------------------------------------------------
  SUBROUTINE deallocate_beam(beam)

    TYPE(beam_block), POINTER :: beam

    IF (ASSOCIATED(beam%profile)) DEALLOCATE(beam%profile)
    IF (ASSOCIATED(beam%phase)) DEALLOCATE(beam%phase)
    !IF (ASSOCIATED(beam%phase)) DEALLOCATE(beam%phase)
    IF (beam%use_profile_function) &
        CALL deallocate_stack(beam%profile_function)
    IF (beam%use_phase_function) &
        CALL deallocate_stack(beam%phase_function)
    IF (beam%use_time_function) &
        CALL deallocate_stack(beam%time_function)
    DEALLOCATE(beam)

  END SUBROUTINE deallocate_beam
  !------------------------------------------------------------------
  ...
  !-----------------------------------------------------------------
  SUBROUTINE deallocate_beams

    TYPE(beam_block), POINTER :: current, next

    current => beam_x_min
    DO WHILE(ASSOCIATED(current))
      next => current%next
      CALL deallocate_beam(current)
      current => next
    ENDDO
  ...
  END SUBROUTINE deallocate_beams
  !--------------------------------------------------------------
  ...
  !--------------------------------------------------------------
  ! Subroutine to attach a created laser object to the correct boundary
  SUBROUTINE attach_beam(beam)

    INTEGER :: boundary
    TYPE(laser_block), POINTER :: beam

    boundary = beam%boundary

    IF (boundary == c_bd_x_min) THEN
      CALL attach_beam_to_list(beam_x_min, beam)
    ELSE IF (boundary == c_bd_x_max) THEN
      CALL attach_beam_to_list(beam_x_max, beam)
    ELSE IF (boundary == c_bd_y_min) THEN
      CALL attach_beam_to_list(beam_y_min, beam)
    ELSE IF (boundary == c_bd_y_max) THEN
      CALL attach_beam_to_list(beam_y_max, beam)
    ENDIF

  END SUBROUTINE attach_beam
  !--------------------------------------------------------------
  ...
  !-------------------------------------------------------------
  FUNCTION beam_time_profile(beam)

    TYPE(beam_block), POINTER :: beam
    REAL(num) :: beam_time_profile
    INTEGER :: err

    err = 0
    IF (beam%use_time_function) THEN
      beam_time_profile = evaluate(beam%time_function, err)
      RETURN
    ENDIF

    beam_time_profile = custom_beam_time_profile(laser)

  END FUNCTION beam_time_profile
  !--------------------------------------------------------------
  ...
  !------------------------------------------------------------- 
  SUBROUTINE beam_update_phase(beam)

    TYPE(beam_block), POINTER :: beam
    INTEGER :: i, err

    err = 0
    SELECT CASE(beam%boundary)
      CASE(c_bd_x_min, c_bd_x_max)
        DO i = 1,ny
          beam%phase(i) = &
              evaluate_at_point(beam%phase_function, 0, i, err)
        ENDDO
      CASE(c_bd_y_min, c_bd_y_max)
        DO i = 1,nx
          beam%phase(i) = &
              evaluate_at_point(beam%phase_function, i, 0, err)
        ENDDO
    END SELECT

  END SUBROUTINE beam_update_phase
  !-------------------------------------------------------------
  ...
  !----------------------------------------------------------
  SUBROUTINE beam_update_profile(beam)
    ...
    SELECT CASE(beam%boundary)
      CASE(c_bd_x_min, c_bd_x_max)
        DO i = 1,ny
          beam%profile(i) = &
              evaluate_at_point(beam%profile_function, 0, i, err)
        ENDDO
      CASE(c_bd_y_min, c_bd_y_max)
        DO i = 1,nx
          beam%profile(i) = &
              evaluate_at_point(beam%profile_function, i, 0, err)
        ENDDO
    END SELECT

  END SUBROUTINE beam_update_profile
  !-----------------------------------------------------------
  ...
  !------------------------------------------------------------
  ! Actually does the attaching of the beam to the correct list
  SUBROUTINE attach_beam_to_list(list, beam)

    TYPE(beam_block), POINTER :: list
    TYPE(beam_block), POINTER :: beam
    TYPE(beam_block), POINTER :: current
  ...
  END SUBROUTINE attach_beam_to_list
  !--------------------------------------------------------------
  ...
  !----------------------------------------------
  SUBROUTINE set_beam_dt

    REAL(num) :: dt_local
    TYPE(beam_block), POINTER :: current

    dt_beam = HUGE(1.0_num)

    current => beam_x_min
    DO WHILE(ASSOCIATED(current))
      dt_local = 2.0_num * pi / current%omega
      dt_beam = MIN(dt_beam, dt_local)
      current => current%next
    ENDDO
    ...
    ! Need at least two iterations per beam period
    ! (Nyquist)
    dt_beam = dt_beam / 2.0_num

  END SUBROUTINE set_beam_dt
  !--------------------------------------------------
